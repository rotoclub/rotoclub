...
class SaleApiLine(models.Model):
    ...
    company_id = fields.Many2one('res.company', related='sale_api_id.company_id', string='Company', required=True, index=True)
    currency_id = fields.Many2one('res.currency', related='sale_api_id.currency_id', store=True, string='Currency', readonly=True)
    ...
    sale_api_id = fields.Many2one('sale.api', string='Sale Api', ondelete='cascade')
    ...
    def product_id_change(self):
        ...
        product = self.product_id.with_context(partner=self.sale_api_id.partner_id, quantity=vals.get('product_uom_qty') or self.product_uom_qty, uom=self.product_uom.id)
        self._compute_tax_id()
        if self.sale_api_id.partner_id:
            vals['price_unit'] = self.env['account.tax']._fix_tax_included_price_company(self.product_id.list_price, product.taxes_id, self.tax_id, self.company_id)
        self.update(vals)
